...
def filter_valid_control_pressures(data: pd.DataFrame, args: argparse.Namespace, tables_dir: Optional[Path] = None, prefix: str = "wls") -> pd.DataFrame:
    ...
    # 使用 _to_numeric 而不是 pd.to_numeric：兼容 '0.8MPa'、'控制压差=1.4' 等带单位/文本的取值。
    cp = _to_numeric(data["控制压差目标值"])
    data["control_pressure"] = cp.astype("float32")
    data["control_pressure_rounded"] = cp.round(args.control_round_digits).astype("float32")
    before = len(data)
    keep = data["control_pressure_rounded"].isin(allowed)
    ...
    if after == 0:
        raw_sample = data["控制压差目标值"].astype(str).head(10).tolist() if "控制压差目标值" in data.columns else []
        raise RuntimeError(
            f"过滤控制压差后没有数据。允许值={allowed}。"
            f"这通常不是没有该列，而是列被误识别或取值是带单位/文本格式。"
            f"请查看 tables/{prefix}_control_pressure_raw_value_counts_top80.csv 和 "
            f"tables/{prefix}_control_pressure_numeric_debug.csv。raw_sample={raw_sample}"
        )
    return data
