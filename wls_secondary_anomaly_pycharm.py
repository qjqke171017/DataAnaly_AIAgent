...
def prepare_wls_columns(data: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    data["control_group"] = data["control_pressure"].round(args.control_round_digits).astype(str)
    data.loc[data["control_pressure"].isna(), "control_group"] = "global"
...
def main() -> None:
    ...
    parser.add_argument("--max_train_rows", type=int, default=500000, help="WLS 训练最多使用多少条正常记录。")
    ...
    parser.add_argument("--control_round_digits", type=int, default=3, help="控制压差目标值分组时的小数位。")
