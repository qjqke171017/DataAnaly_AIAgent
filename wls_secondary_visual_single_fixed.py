# -*- coding: utf-8 -*-
"""
WLS 二次侧机理异常检测：单次补充可视化版

核心功能
1. 按控制压差目标值 0.8 / 1.4 / 1.65 分组建模与评价，避免浮点数精确比较错误。
2. 默认只运行 1 次，用于快速补充报告所需的散点图、残差图、score 分布图和设备级图；如需重复实验，可把配置区 N_REPEATS_CONFIG 改大。
3. 同时评估三种 WLS 机理模型：
...
5. 默认不输出逐记录预测结果，只输出汇总指标与设备级结果，避免 2000 万级记录占用大量磁盘空间。
...
- M/S/U 默认只打分和输出设备级统计，不参与主二分类指标。
- 同一设备内用于拟合的样本抽样方式：在每次重复中，先按设备划分 normal_train；然后对每个 normal_train 设备、每个控制压差组，在满足高转速过滤和变量有效的记录中，均匀随机无放回抽取不超过 TRAIN_MAX_ROWS_PER_DEVICE_PER_CONTROL_CONFIG 条记录用于 WLS 拟合。这样可避免长时间序列设备支配回归斜率。
"""
...
SKIP_M_S_U_CONFIG = False          # False=读取 M/S/U 并打分但不参与主指标；True=完全跳过
...
# 重复实验配置
# 本版主要用于补图，默认只跑 1 次；如果仍想重复实验，可改成 20。
N_REPEATS_CONFIG = 1
...
# WLS 高转速剔除：若转速为 0-100，则剔除 speed >= 95；若为 0-1，则剔除 speed >= 0.95。
...
DATA_MAX_ROWS_PER_FILE_CONFIG = None
CSV_CHUNKSIZE_CONFIG = 200000

# 用于 WLS 拟合的训练样本上限：每个训练设备、每个控制压差组最多抽取这些记录。
TRAIN_MAX_ROWS_PER_DEVICE_PER_CONTROL_CONFIG = 500
...
def speed_exclusion_mask(speed_raw: pd.Series, wls_max_speed: float) -> pd.Series:
    q95 = speed_raw.dropna().quantile(0.95) if speed_raw.notna().any() else np.nan
    if pd.notna(q95) and q95 > 2:
        thr = wls_max_speed
    else:
        thr = wls_max_speed / 100.0
    return speed_raw >= thr
    ...
    for code_dir in code_dirs:
        ...
        for fp in csv_files:
            try:
                ...
                df["excluded_high_speed_for_wls"] = speed_exclusion_mask(df["speed_raw"], args.wls_max_speed)
...
def fit_models_for_repeat(data: pd.DataFrame, split: pd.Series, model_id: str, args: argparse.Namespace) -> Tuple[Dict[str, Dict[str, object]], pd.DataFrame]:
    ...
    for grp in sorted(data["control_group"].dropna().astype(str).unique()):
        mask = (
            ...
            (~data["excluded_high_speed_for_wls"].astype(bool)) &
            data["speed_sq"].notna() &
...
def score_model(data: pd.DataFrame, model_id: str, models: Dict[str, Dict[str, object]], args: argparse.Namespace) -> pd.DataFrame:
    ...
    valid_common = (~data["excluded_high_speed_for_wls"].astype(bool)) & data["speed_sq"].notna() & data[target_col].notna()
...
def build_metrics_and_device_summary(
    ...
    for q in quantiles:
        ...
        # 设备级统计。
        dev_tmp = data[["source_code", "device_name", "device_key", "control_group", "binary_label", "label_name", "row_id", "excluded_high_speed_for_wls"]].copy()
        ...
        for (code, dev, grp), sub in dev_tmp.groupby(["source_code", "device_name", "control_group"], observed=True):
            ...
            device_rows.append({
                ...
                "excluded_high_speed_count": int(s["excluded_high_speed_for_wls"].astype(bool).sum()),
...
def summarize_repeats(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    for key, sub in metrics.groupby(group_cols, dropna=False):
        ...
        row["repeat_count"] = int(sub["repeat"].nunique())
...
def plot_metric_summary(summary: pd.DataFrame, fig_dir: Path) -> None:
    ...
    for model_id, mdf in sub.groupby("model_id"):
        ...
        ax.set_title(f"{model_id}：重复实验平均指标 - heldout/all")
        ax.set_xlabel("阈值分位数")
        ax.set_ylabel("指标均值")
        ...
        ax.set_title(f"{model_id}：FPR-Recall 权衡（重复均值）")
        ax.grid(True, alpha=0.3)
        fig.tight_layout()
...
def plot_diagnostic_for_repeat0(data: pd.DataFrame, split: pd.Series, model_id: str, model_name: str, models: Dict[str, Dict[str, object]], score_df: pd.DataFrame, thresholds: Dict[str, Dict[float, float]], args: argparse.Namespace, fig_dir: Path, html_dir: Path) -> None:
    ...
    base = base[base["score"].notna()].copy()
    if base.empty:
        return
    base["真实类别"] = np.where(base["binary_label"].eq(0), "正常", np.where(base["binary_label"].eq(1), "二次侧异常", "未标注"))
    # 修正旧版绘图报错：base 已经过 score.notna() 过滤，行数可能小于原始 data。
    # 必须按索引对齐 split，而不能直接使用 split.values。
    base["split"] = split.reindex(base.index).astype(str)
    ...
    for grp in sorted(base["control_group"].astype(str).unique()):
        ...
        # score 分布
        fig, ax = plt.subplots(figsize=(10, 6))
        normal = sub_full[sub_full["binary_label"].eq(0)]["score"]
        anomaly = sub_full[sub_full["binary_label"].eq(1)]["score"]
        if len(normal):
            ax.hist(normal, bins=80, alpha=0.55, density=True, label=f"正常 n={len(normal):,}")
        if len(anomaly):
            ax.hist(anomaly, bins=80, alpha=0.55, density=True, label=f"异常 n={len(anomaly):,}")
        ...
        add_count_box(ax, count_box_text(sub_full, sub_full["score"]))
...
def plot_device_top(device_df: pd.DataFrame, fig_dir: Path, html_dir: Path, q: float) -> None:
    if device_df.empty:
        return
    sub = device_df[np.isclose(device_df["threshold_quantile"], q)].copy()
    if sub.empty:
        return
    # 只画 repeat=0，避免图过多；总表仍保留所有重复。
    sub = sub[sub["repeat"].eq(0)].copy()
    for model_id, mdf in sub.groupby("model_id"):
        top = mdf.sort_values("pred_abnormal_rate", ascending=False).head(30)
        if top.empty:
            continue
        fig, ax = plt.subplots(figsize=(12, 8))
        labels = (top["device_key"].astype(str) + " | cp=" + top["control_group"].astype(str)).iloc[::-1]
        ax.barh(labels, top["pred_abnormal_rate"].values[::-1])
        ax.set_xlabel("设备异常率")
        ax.set_title(f"{model_id} Top 30 设备异常率（repeat=0, q={q:.4f}）")
        ax.grid(True, axis="x", alpha=0.3)
        fig.tight_layout()
        fig.savefig(fig_dir / f"top30_device_abnormal_rate_{model_id}_q{int(q*10000):04d}.png", dpi=220)
        plt.close(fig)

        topc = mdf.sort_values("max_consecutive_abnormal", ascending=False).head(30)
        fig, ax = plt.subplots(figsize=(12, 8))
        labels = (topc["device_key"].astype(str) + " | cp=" + topc["control_group"].astype(str)).iloc[::-1]
        ax.barh(labels, topc["max_consecutive_abnormal"].values[::-1])
        ax.axvline(10, linestyle="--", linewidth=1, label="连续误报/异常 10 次参考线")
        ax.set_xlabel("最大连续判异常次数")
        ax.set_title(f"{model_id} Top 30 最大连续异常次数（repeat=0, q={q:.4f}）")
        ax.grid(True, axis="x", alpha=0.3)
        ax.legend()
        fig.tight_layout()
        fig.savefig(fig_dir / f"top30_device_max_consecutive_{model_id}_q{int(q*10000):04d}.png", dpi=220)
        plt.close(fig)

        if PLOTLY_AVAILABLE:
            hdir = html_dir / "device_top"
            hdir.mkdir(parents=True, exist_ok=True)
            top_html = mdf.sort_values("pred_abnormal_rate", ascending=False).head(300)
            fig = px.bar(
                top_html.sort_values("pred_abnormal_rate"),
                x="pred_abnormal_rate", y="device_key", color="source_code", orientation="h",
                hover_data=["control_group", "valid_scored_records", "pred_abnormal_count", "max_consecutive_abnormal", "mean_score", "p95_score"],
                title=f"{model_id} 设备异常率 Top 300（repeat=0, q={q:.4f}）",
            )
            fig.write_html(hdir / f"device_abnormal_rate_top300_{model_id}.html")
...
def main() -> None:
    ...
    if (not args.root_dir) or ("请把这里改成" in str(args.root_dir)):
        raise ValueError("请先在代码顶部修改 DATA_ROOT_DIR 为你的数据根目录。")
    if (not args.output_dir) or ("请把这里改成" in str(args.output_dir)):
        raise ValueError("请先在代码顶部修改 OUTPUT_DIR 为你的输出目录。")
    ...
    if not avg_metrics.empty:
        ...
        if not quick.empty:
            quick = quick.sort_values(["meets_fpr_lt_0_05_recall_gt_0_50_mean", "fpr_mean", "recall_mean"], ascending=[False, True, False])
    ...
    if not avg_metrics.empty:
        try:
            ...
            print(f"[WARN] 汇总指标图生成失败，但表格已输出：{e}")
    for model_id, model_name, models, score_df, thresholds, split0 in repeat0_plot_payload:
        try:
            ...
            print(f"[WARN] {model_id} 诊断散点/残差图生成失败，继续生成其他图：{e}")
    if not device_all.empty:
        try:
            plot_device_top(device_all, fig_dir, html_dir, args.main_plot_quantile)
        except Exception as e:
            print(f"[WARN] 设备级图生成失败，但设备级表格已输出：{e}")
    ...
    print("    wls_average_metrics_over_repeats.csv          # 重复实验均值总表")
