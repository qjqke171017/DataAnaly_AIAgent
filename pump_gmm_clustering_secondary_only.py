...
6. GMM 只使用二次侧指标和二次侧派生指标，不使用一次侧指标；
7. 聚类/打分结束后，再按设备汇总异常率、最大异常分数等指标。
...
class Config:
    ...
    # 3) 特征配置
    # --------------------------
    # None 表示自动选择二次侧连续数值型特征；
    # 如果想手动指定，就写：FEATURE_COLUMNS=["二次侧泵转速", "二次侧泵压差", ...]
    FEATURE_COLUMNS: Optional[List[str]] = None

    # 强制只使用二次侧指标。
    # True 时：自动选特征会跳过所有非“二次侧/二次/派生_二次侧”列；
    # 手动 FEATURE_COLUMNS 中如果写入一次侧列，也会被过滤掉。
    FORCE_SECONDARY_ONLY: bool = True

    # 是否加入脚本自动构造的二次侧物理派生特征。
...
def is_secondary_feature_name(col: object) -> bool:
    """判断一个列名是否属于二次侧指标或二次侧派生指标。"""
    name = str(col).strip()
    if name.startswith("派生_二次侧"):
        return True
    if "二次侧" in name or "二次" in name:
        return True
    return False


def add_derived_features(df: pd.DataFrame, resolved_cols: Dict[str, Optional[str]]) -> pd.DataFrame:
    """只加入二次侧物理派生特征。能构造则构造，不能构造则自动跳过。"""
    out = df.copy()
...
def auto_select_feature_columns(df: pd.DataFrame, cfg: Config) -> Tuple[List[str], pd.DataFrame]:
    ...
    for col in df.columns:
        ...
        if cfg.FORCE_SECONDARY_ONLY and (not is_secondary_feature_name(col)):
            rows.append({"feature": col, "selected": False, "reason": "非二次侧指标", "missing_rate": np.nan, "numeric_rate": np.nan, "n_unique": np.nan})
            continue
...
def prepare_feature_table(group_df: pd.DataFrame, cfg: Config) -> Tuple[pd.DataFrame, List[str], pd.DataFrame, Dict[str, Optional[str]]]:
    ...
    if cfg.FEATURE_COLUMNS is None:
        ...
        original_manual = list(cfg.FEATURE_COLUMNS)
        existing_manual = [c for c in original_manual if c in data.columns]
        missing = [c for c in original_manual if c not in data.columns]

        if cfg.FORCE_SECONDARY_ONLY:
            feature_cols = [c for c in existing_manual if is_secondary_feature_name(c)]
            filtered_non_secondary = [c for c in existing_manual if not is_secondary_feature_name(c)]
        else:
            feature_cols = existing_manual
            filtered_non_secondary = []

        report_rows = []
        for c in original_manual:
            if c not in data.columns:
                selected = False
                reason = "手动指定但原表不存在"
            elif cfg.FORCE_SECONDARY_ONLY and not is_secondary_feature_name(c):
                selected = False
                reason = "手动指定但被过滤：非二次侧指标"
            else:
                selected = True
                reason = "手动指定"
            report_rows.append({"feature": c, "selected": selected, "reason": reason})
        feature_report = pd.DataFrame(report_rows)

        if missing:
            print("警告：以下手动指定特征在数据中不存在，已跳过：", missing)
        if filtered_non_secondary:
            print("警告：以下手动指定特征不是二次侧指标，已按 FORCE_SECONDARY_ONLY=True 跳过：", filtered_non_secondary)
